...
use crate::{
    api::jellyfin::{Album, Artist, Track, UserStats},
...
pub fn head(title: &str) -> Markup {
    html! {
        ...
        script defer src="static/script/maze-background.js" {}
...
pub fn navbar(items: &[(&str, &str)]) -> Markup {
    html! {
            ...
            hx-push-url="true"
        {
            ...
            button.nav-link.maze-toggle #maze-toggle
                type="button"
                aria-pressed="false"
            { "View maze" }
            button.nav-link.maze-toggle #maze-disable
                type="button"
                aria-pressed="false"
            { "Disable maze" }
        }
...
pub fn ascii_banner() -> Markup {
    let banner =
        load_text_from_file("./static/ascii.txt").unwrap_or("Couldn't load banner.".into());
    html! { pre.ascii-banner { (banner) } }
}

pub fn welcome_message() -> Markup {
    let message = load_text_from_file("./static/welcome.txt")
        .unwrap_or("Couldn't load welcome message".into());
    ...
    html! {
        div.flex-row.gap4 {
            a.center.border.flex-grow href="https://twitch.tv/mentaalachtergesteld" target="_blank" rel="noopener noreferrer" {
                img src="static/img/twitch.svg" alt="Twitch" height="24";
            }

            a.center.border.flex-grow href="https://git.mentaal.eu/mentaal" target="_blank" rel="noopener noreferrer" {
                img src="static/img/gitea.svg" alt="Gitea" height="24";
            }
        }
    }
}

pub fn badge_wall() -> Markup {
    html! {
        div.badge-wall.flex-row.gap4.align-center.justify-center {
            img src="static/img/badges/2020-02-29-gnu-linux.png" alt="Made on GNU/Linux";
            img src="static/img/badges/2020-02-29-neovim.png" alt="Made with Neovim";
            img src="static/img/badges/cli.gif" alt="Command-line interface";
            img src="static/img/badges/europe.png" alt="Europe";
            img src="static/img/badges/firefox.gif" alt="Firefox";
            img src="static/img/badges/get_linux_suse.gif" alt="Get Linux: openSUSE";
            img src="static/img/badges/best-viewed.gif" alt="Best viewed with any browser";
            img src="static/img/badges/kde.png" alt="KDE";
            img src="static/img/badges/linux.png" alt="Linux";
            img src="static/img/badges/no-windows.gif" alt="No Windows";
            img src="static/img/badges/palestine.gif" alt="Palestine";
            img src="static/img/badges/vim.png" alt="Vim";
        }
    }
}
...
pub fn api_error(message: &str, endpoint: &str, is_inline: bool) -> Markup {
    lazy_component(
        Some(html! { span { (message) } }),
        html! {},
        endpoint,
        "1m",
        is_inline,
...
pub fn now_playing(data: Option<&Option<Track>>) -> Markup {
    ...
    lazy_component(
        content,
        skeleton_span("loading current track..."),
        "/comp/now-playing",
        "15s",
        true,
    )
...
pub fn top_artists(data: Option<&Vec<Artist>>) -> Markup {
    ...
    lazy_component(
        content,
        skeleton_list("loading artist...", 10),
        "/comp/top-artists",
        "60m",
        false,
    )
...
pub fn top_tracks(data: Option<&Vec<Track>>) -> Markup {
    let content = data.map(|tt| {
        html! {
            div.flex-column.gap4 { @for (i, track) in tt.iter().enumerate() {
                div.list-row title=(format!("{} - {}", track.name, track.artist)) {
                    span.rank-col { (i+1) "." }
    ...
    lazy_component(
        content,
        skeleton_list("loading track...", 10),
        "/comp/top-tracks",
        "60m",
        false,
    )
...
pub fn top_albums(data: Option<&Vec<Album>>) -> Markup {
    ...
    lazy_component(
        content,
        skeleton_list("loading album...", 10),
        "/comp/top-albums",
        "60m",
        false,
    )
...
pub fn jellyfin_user_stats(data: Option<&UserStats>) -> Markup {
    ...
    lazy_component(content, placeholder, "/comp/user-stats", "60m", false)
}

pub fn jellyfin_stats() -> Markup {
    html! {
...
pub fn message_item(msg: &Message, admin_csrf_token: Option<&str>) -> Markup {
    ...
    html! {
        div.border.message.font-small {
            ...
            @if let Some(csrf_token) = admin_csrf_token {
                button.admin-delete-btn
                    type="button"
                    hx-delete=(format!("/comp/messages/{}", msg.id))
                    hx-target="closest .message"
                    hx-swap="outerHTML"
                    hx-confirm="Delete this guestbook message?"
...
pub fn message_list(
    ...
    html! {
        div.flex-column.gap4 #message-list {
            @for msg in messages {
                (message_item(msg, admin_csrf_token))
            }
